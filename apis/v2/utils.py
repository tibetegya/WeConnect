...
def validate_user_payload(new_user):
    ...
    if not ('user_name' in new_user.keys()
            and 'email' in new_user.keys() and 'password' in new_user.keys()):
...
def validate_business_payload(new_payload):
    ...
    if not ('business_name' in new_payload.keys()
            and 'category' in new_payload.keys()
            and 'location' in new_payload.keys() and 'profile' in new_payload.keys()):
...
def validate_business_update_payload(new_payload):
    ...
    if not ('business_name' in new_payload.keys()
            or 'category' in new_payload.keys()
            or 'location' in new_payload.keys() or 'profile' in new_payload.keys()):

            return {'message': 'Payload should have at least one field'}, 403

    if 'business_name' in new_payload.keys():
    ...
    if 'category' in new_payload.keys():
    ...
    if 'location' in new_payload.keys():
    ...
    if 'profile' in new_payload.keys():
